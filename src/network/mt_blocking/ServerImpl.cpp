...
void ServerImpl::Stop() {
	...
	for (int _client_socket: _sockets_active) {
		shutdown(_client_socket, SHUT_RD);
	}
    }
}
...
void ServerImpl::Worker(int client_socket) {
	...
	// no input from this client
	{
	    std::unique_lock<std::mutex> lock(_m);
	    _sockets_active.erase(client_socket);
	    close(client_socket);
	    
	    // if there are no active clients AND we sent output to all received commands we can stop the process
	    if (_sockets_active.empty() && !running.load()) {
		    cv.notify_all();
	    }
	}
...
void ServerImpl::OnRun() {
    ...
    std::string argument_for_command;
    std::unique_ptr<Execute::Command> command_to_execute;
    Concurrency::Executor executor(4, _max_workers);
    while (running.load()) {
    ...
    _logger->warn("Network stopped");
        ...
        //{
	// скопировать сюда цикл из st_blocking
	// должен быть лимит на клиентов
	// Stop: прекращаем принимать новых клиентов и новые команды от старых, но должны отправить существующим клиентам ответ на старые команды
	// Join: вызывающий поток блокирует до тех пор, пока весь сетевой слой не остановится до конца (закрыли всех клиентов, все почистили)
	// Надо дописать Stop, Join и в OnRun порождение треда
	// shutdown: прекращаем принимать новых клиентов shutdown(_server_socket, SHUT_RDWR), прекратить новые команды от клиентов - shutdown(_client_socket, SHUT_RD) (чтение начнет возвращать ошибку)
	// нужен будет mutex и cd
